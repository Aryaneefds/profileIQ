      ...
      <nav className="p-4 space-y-1">
        {links.map(link => (
          <NavLink
            key={link.to}
            to={link.to}
            end={link.to === '/student' || link.to === '/counselor'}
            className={({ isActive }) => `
            ...
            `}
          >
            <span className={`transition-colors duration-200 ${link.to === location.pathname ? 'text-zinc-900' : 'text-zinc-400 group-hover:text-zinc-600'}`}>
              {link.icon}
            </span>
